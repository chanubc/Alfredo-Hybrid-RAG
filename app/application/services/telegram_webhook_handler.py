...
class TelegramWebhookHandler:
    ...
    async def handle(self, data: dict, background_tasks: BackgroundTasks) -> None:
        ...
        if telegram_id is None:
            logger.warning("Cannot determine telegram_id from message: {}", message)
            return
        logger.info("Received message from {}: {}", telegram_id, text)
        ...
        if urls:
            for url in urls:
                logger.info("Processing URL from {}: {}", telegram_id, url)
    ...
    async def _handle_callback(self, callback: dict) -> None:
        ...
        if data == "help":
            ...
            try:
                ...
                logger.warning("mark_read callback failed: {}", exc)
