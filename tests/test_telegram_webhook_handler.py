...
@pytest.mark.asyncio
async def test_handle_url_message_schedules_save_link_usecase(
...
@pytest.mark.asyncio
async def test_help_callback_routes_to_telegram_help(webhook_handler, webhook_dependencies, background_tasks):
...
async def test_menu_callbacks_send_guidance_messages(
    webhook_handler, webhook_dependencies, callback_data, expected_text, background_tasks
):
    ...
    reply_markup = webhook_dependencies["telegram"].send_message.await_args.kwargs["reply_markup"]
    assert reply_markup["inline_keyboard"][-1] == [{"text": "« Back to Menu", "callback_data": "nav:menu"}]


@pytest.mark.asyncio
async def test_menu_report_callback_routes_to_report_command(
    webhook_handler, webhook_dependencies, background_tasks
):
...
@pytest.mark.asyncio
async def test_nav_menu_callback_routes_to_menu_command(webhook_handler, webhook_dependencies, background_tasks):
    callback = {"id": "callback-nav", "data": "nav:menu", "from": {"id": 123}}

    await webhook_handler._handle_callback(callback, background_tasks)

    background_tasks.add_task.assert_called_once_with(webhook_dependencies["message_router"].route, 123, "/menu")
...
async def test_mark_read_callback_uses_usecase_and_sends_success_message(
    ...
    webhook_dependencies["telegram"].send_message.assert_awaited_once()
    assert webhook_dependencies["telegram"].send_message.await_args.args == (123, "✅ 읽음 처리되었습니다.")
    assert webhook_dependencies["telegram"].send_message.await_args.kwargs["reply_markup"]["inline_keyboard"][-1] == [
...
async def test_mark_read_callback_sends_not_found_message_when_usecase_returns_false(
    ...
    webhook_dependencies["telegram"].send_message.assert_awaited_once()
    assert webhook_dependencies["telegram"].send_message.await_args.args == (123, "링크를 찾을 수 없습니다.")
    assert webhook_dependencies["telegram"].send_message.await_args.kwargs["reply_markup"]["inline_keyboard"][-1] == [
        {"text": "« Back to Menu", "callback_data": "nav:menu"}
    ]
